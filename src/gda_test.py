...
positive_data = [np.random.default_rng().multivariate_normal(mu1 , s1) for i in range(100)]
negative_data = [np.random.default_rng().multivariate_normal(mu0 , s0) for i in range(100)]
model = GDA.GDA(positive_data , negative_data)
print(model)
test_positive_data = [np.random.default_rng().multivariate_normal(mu1 , s1) for i in range(100)]
test_negative_data = [np.random.default_rng().multivariate_normal(mu0 , s0) for i in range(100)]
